...
static int RNG_HealthTestLocal(int reseed);
...
static int RNG_HealthTestLocal(int reseed)
{
    ...
    if (reseed) {
        ret = RNG_HealthTest(1, entropyA, sizeof(entropyA),
        ...
        ret = RNG_HealthTest(0, entropyB, sizeof(entropyB),
                                NULL, 0,
                                check, sizeof(check));
        if (ret == 0) {
